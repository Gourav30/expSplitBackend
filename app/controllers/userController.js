...
let forgotPassword = (req, res) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (newUserObj) {
                ...
                transporter.sendMail(mailOptions, function (err, data) {
                    if (err) {
                        logger.error('some error occured', 'sendmail', 9)
    ...
    findUser(req, res)
        ...
        .then((resolve) => {
            let apiResponse = response.generate(false, 'Reset Token sent to your Email', 200, resolve)
            res.status(200)
            res.send(apiResponse)
        })
        .catch((err) => {
...
let logout = (req, res) => {
    ...
    UserModel.findOneAndRemove({ userId: req.user.userId }, (err, result) => {
        if (err) {
            console.log(err)
